...
#[Route('/reservation/{id}/candidature', name: 'candidature_prestataire', methods: ['POST'])]
    public function addCandidature(int $id, Request $request, ReservationRepository $reservationRepository, PrestataireRepository $prestataireRepository): JsonResponse
    {
        ...
        $prestataireId = $data['prestataire_id'] ?? null;

        if (!$prestataireId) {
            return new JsonResponse(['error' => 'Prestataire ID is missing'], Response::HTTP_BAD_REQUEST);
        }
        ...
        // Logique pour ajouter la candidature
        // Par exemple, ajouter le prestataire à une liste de candidats pour la réservation
        $reservation->addCandidat($prestataire);

        $this->entityManager->flush();

        return new JsonResponse(['status' => 'Candidature added successfully'], Response::HTTP_OK);
    }
