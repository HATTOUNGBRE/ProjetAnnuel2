...
class PropertyController extends AbstractController
{
    ...
    public function searchProperties(Request $request, PropertyRepository $propertyRepository, SerializerInterface $serializer, LoggerInterface $logger): JsonResponse
    {
        ...
        if ($maxPersons) {
            $criteria['maxPersons'] = $maxPersons;
        }

        if (empty($criteria)) {
            return new JsonResponse(['message' => 'No search criteria provided'], JsonResponse::HTTP_BAD_REQUEST);
        }

        $logger->info('Searching for properties with criteria: ' . json_encode($criteria));

        $properties = $propertyRepository->findByCriteria($criteria);

        if (empty($properties)) {
            $logger->info('No properties found for the given criteria');
        } else {
