...
class PropertyController extends AbstractController
{
    ...
    public function searchProperties(Request $request, PropertyRepository $propertyRepository): JsonResponse
    {
        ...
        if ($commune) {
            $queryBuilder->andWhere('p.commune LIKE :commune')
                ->setParameter('commune', '%' . $commune . '%');
        }

        if ($maxPersons) {
            $queryBuilder->andWhere('p.maxPersons >= :maxPersons')
                ->setParameter('maxPersons', $maxPersons);
        }

        $properties = $queryBuilder->getQuery()->getResult();

        return new JsonResponse($properties, JsonResponse::HTTP_OK, [], ['groups' => 'property:read']);
    }
}
