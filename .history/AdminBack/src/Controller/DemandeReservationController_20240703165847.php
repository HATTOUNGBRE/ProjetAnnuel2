...
public function acceptDemandeReservation(int $id): JsonResponse
{
    $demande = $this->getDoctrine()->getRepository(DemandeReservation::class)->find($id);
    if (!$demande) {
        return new JsonResponse(['message' => 'Demande not found'], JsonResponse::HTTP_NOT_FOUND);
    }
    ...
    $reservation->setGuestNb($demande->getGuestNb());
    $reservation->setProperty($demande->getProperty());
    $reservation->setVoyageur($demande->getVoyageur()); // Set the voyageur

    $this->entityManager->persist($reservation);
    $this->entityManager->flush();

    return new JsonResponse(['message' => 'Demande accepted and reservation created'], JsonResponse::HTTP_OK);
}
