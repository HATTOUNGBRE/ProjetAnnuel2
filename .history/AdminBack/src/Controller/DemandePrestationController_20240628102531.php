...
use App\Repository\UserRepository; // Ajouter cette ligne
...
class DemandePrestationController extends AbstractController
{
    ...
    public function accept(
        int $id,
        DemandePrestationRepository $demandePrestationRepository,
        EntityManagerInterface $entityManager,
        ...
        // Calculer la date de fin en ajoutant un jour à la date d'effet
        $dateDEffet = $demande->getDateDEffet();
        $dateDeFin = (clone $dateDEffet)->modify('+1 day');

        $prestation = new Prestation();
        $prestation->setTitre($demande->getTitre());
        $prestation->setDescription($demande->getDescription());
        $prestation->setDateDEffet($dateDEffet);
    ...
    #[Route('/api/demande-prestations/{id}/cancel', name: 'cancel_demande_prestation', methods: ['POST'])]
    public function cancel(int $id, DemandePrestationRepository $demandePrestationRepository, EntityManagerInterface $entityManager): JsonResponse
    {
        $demande = $demandePrestationRepository->find($id);
        if (!$demande) {
            return new JsonResponse(['message' => 'Demande not found'], 404);
        ...
        return new JsonResponse(['message' => 'Demande annulée'], 200);
    }

    #[Route('/api/user-demandes-prestations/{userId}', name: 'user_demandes_prestations', methods: ['GET'])]
    public function getUserDemandes(int $userId, DemandePrestationRepository $demandePrestationRepository): JsonResponse
    {
