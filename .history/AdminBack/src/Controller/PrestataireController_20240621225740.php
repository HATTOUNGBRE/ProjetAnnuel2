...
#[Route('/prestataire/{userId}/disponibilites', name: 'get_disponibilites', methods: ['GET'])]
public function getDisponibilites(int $userId): JsonResponse
{
    $prestataire = $this->entityManager->getRepository(Prestataire::class)->findOneBy(['user' => $userId]);

    if (!$prestataire) {
        return new JsonResponse(['error' => 'Prestataire not found'], Response::HTTP_NOT_FOUND);
    }

    $disponibilites = $prestataire->getDisponibilites(); // Assuming you have a method to get disponibilites
...
#[Route('/prestataire/{userId}/disponibilites', name: 'add_disponibilite', methods: ['POST'])]
public function addDisponibilite(Request $request, int $userId): JsonResponse
{
    $data = json_decode($request->getContent(), true);
    ...
    // Validate and create new Disponibilite entity
    $disponibilite = new Disponibilite();
    $disponibilite->setStart(new \DateTime($data['start']));
    $disponibilite->setEnd(new \DateTime($data['end']));
    $disponibilite->setTitle($data['title']);

    $prestataire->addDisponibilite($disponibilite);

    $this->entityManager->persist($disponibilite);
    $this->entityManager->flush();

    return new JsonResponse(['message' => 'Disponibilite added successfully'], Response::HTTP_CREATED);
}
