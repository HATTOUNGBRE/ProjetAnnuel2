...
class PropertyController extends AbstractController
{
    ...
    public function searchProperties(Request $request, PropertyRepository $propertyRepository, LoggerInterface $logger): JsonResponse
    {
        $commune = $request->query->get('commune');
    
        // Remove the surrounding double quotes if they exist
        if (substr($commune, 0, 1) === '"' && substr($commune, -1) === '"') {
            $commune = substr($commune, 1, -1);
        }
        ...
        $logger->info('Searching for properties in commune: ' . $commune);
