...
const Account = ({ userId }) => {
  ...
  const fetchPrestataireDetails = async (userId) => {
    console.log('Fetching prestataire details for user ID:', userId);
    try {
      ...
      console.log('Prestataire details fetched:', data);
    } catch (error) {
      setError(error.message);
    }
  };
  ...
  useEffect(() => {
    console.log('useEffect called with userId:', userId, 'isLoggedIn:', isLoggedIn);
    if (userId) {
      setLoading(true);
      fetchUser(userId);
      ...
      fetchUserDetails().then((fetchedUserId) => {
        if (fetchedUserId) {
          setUserId(fetchedUserId);
          fetchUser(fetchedUserId);
          fetchPrestataireDetails(fetchedUserId);
        } else {
          setLoading(false);
