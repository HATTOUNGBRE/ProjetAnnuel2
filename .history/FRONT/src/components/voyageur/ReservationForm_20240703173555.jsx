...
const ReservationForm = () => {
  ...
  const isDateUnavailable = (date) => {
    return unavailableDates.some(reservation =>
      (new Date(reservation.dateArrivee) <= new Date(date) && new Date(reservation.dateDepart) >= new Date(date))
    );
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (isDateUnavailable(dateArrivee) || isDateUnavailable(dateDepart)) {
      ...
      <form onSubmit={handleSubmit}>
        <div className="mb-4">
          <label className="block text-gray-700 font-bold mb-2">
            Date d'arrivée:
            <input type="date" value={dateArrivee} onChange={handleDateArriveeChange} required className={`w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 ${isDateUnavailable(dateArrivee) ? 'bg-red-200' : ''}`} />
          </label>
        </div>
        <div className="mb-4">
          <label className="block text-gray-700 font-bold mb-2">
            Date de départ:
            <input className={`w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 ${isDateUnavailable(dateDepart) ? 'bg-red-200' : ''}`} type="date" value={dateDepart} onChange={handleDateDepartChange} required />
          </label>
        </div>
        <div className="mb-4">
          <label className="block text-gray-700 font-bold mb-2">
