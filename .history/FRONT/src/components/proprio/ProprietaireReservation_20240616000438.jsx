...
const ProprietaireReservation = () => {
  ...
  useEffect(() => {
    const fetchReservations = async () => {
      try {
        const response = await fetch('http://localhost:8000/api/reservations'); // Mettre à jour l'URL avec votre endpoint réel
  ...
  const pendingReservations = reservations.filter(reservation => reservation.statut === 'En attente');
