...
const PrestataireDashboard = () => {
  ...
  const rejectPrestation = async (prestationId) => {
    try {
      const response = await fetch(`http://localhost:8000/api/prestations/${prestationId}/reject`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        // Refresh the list of prestations after rejecting
        fetchPrestations();
      } else {
        console.error('Error rejecting prestation');
      }
    } catch (error) {
      console.error('Error rejecting prestation:', error);
    }
  };

  const fetchPrestations = async () => {
    try {
      const response = await fetch('http://localhost:8000/api/prestations/unassigned');
      const data = await response.json();
      setPrestations(data);
      setLoading(false);
    } catch (error) {
              ...
              {pendingPrestations.map(prestation => (
                <div key={prestation.id} className="p-4 bg-white rounded-lg shadow-md">
                  ...
                  <button
                    ...
                    onClick={() => rejectPrestation(prestation.id)}
                  >
                    Refuser
                  </button>
                </div>
