...
const ProprietaireDashboard = () => {
    ...
    <h2 className="text-2xl font-semibold text-gray-800 mb-4">Demandes de Réservation ({activeDemandes.length})</h2>
    {successMessage && (
    ...
    {activeDemandes.length === 0 ? (
        ...
        <ul className='dmd_active'>
            {activeDemandes.map(demande => {
                const isInProgress = demande.status === 'Accepted' && new Date(demande.dateArrivee) <= new Date() && new Date(demande.dateDepart) >= new Date();
                const isAcceptedButNotStarted = demande.status === 'Accepted' && new Date(demande.dateArrivee) > new Date();
                return (
                    <li key={demande.id} className="bg-white shadow-md rounded-lg p-4 mb-4">
                        ...
                        {demande.status === 'En attente' && (
                            <>
                                ...
                                <button
                                    ...
                                    className="bg-red-500 text-white px-4 py-2 rounded mt-2 hover:bg-red-600 ml-2"
                                >
                                    Annuler
                                </button>
                            </>
                        )}
                        {isAcceptedButNotStarted && (
                            <>
                                <button
                                    className="bg-gray-500 text-white px-4 py-2 rounded mt-2 cursor-not-allowed"
                                    disabled
                                >
                                    En cours
                                </button>
                        ...
                        {isInProgress && (
                            <p className="text-red-500">Cette demande est en cours et ne peut pas être annulée.</p>
                        )}
                    </li>
                );
            })}
