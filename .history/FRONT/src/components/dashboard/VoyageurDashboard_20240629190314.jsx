    ...
    const confirmCancel = async () => {
        if (demandeToCancel) {
            try {
                ...
                if (response.ok) {
                    const updatedDemandes = demandes.filter(d => d.id !== demandeToCancel.id);
                    setDemandes(updatedDemandes);

                    const updatedHistorique = historique.map(h => (
                        h.voyageurId === demandeToCancel.voyageurId && 
                        h.property.id === demandeToCancel.property.id && 
                        h.dateArrivee === demandeToCancel.dateArrivee &&
                        h.dateDepart === demandeToCancel.dateDepart
                    ) ? { ...h, status: 'Annulée' } : h);
