...
const ProprietaireDashboard = () => {
    ...
    const handleCancel = async (demandeId) => {
        try {
            const response = await fetch(`http://localhost:8000/api/demandes/${demandeId}/cancel`, {
                method: 'POST',
            });

            if (!response.ok) {
                throw new Error('Failed to cancel the demande');
            }

            setSuccessMessage('La demande a été annulée avec succès.');
            console.log('Cancelled demande with id:', demandeId);
            ...
            console.error('Error cancelling the demande:', error);
        }
    };

    const handleReject = async (demandeId) => {
        try {
            const response = await fetch(`http://localhost:8000/api/demandes/${demandeId}/reject`, {
                method: 'POST',
            });

            if (!response.ok) {
                throw new Error('Failed to reject the demande');
            }

            setSuccessMessage('La demande a été refusée avec succès.');
            console.log('Rejected demande with id:', demandeId);
            ...
            console.error('Error rejecting the demande:', error);
        }
    };
                    ...
                    {activeDemandes.length === 0 ? (
                        ...
                        <ul className='dmd_active'>
                            {activeDemandes.map(demande => {
                                ...
                                const startDate = new Date(demande.dateArrivee);
                                const endDate = new Date(demande.dateDepart);
                                    ...
                                    <li key={demande.id} className="bg-white shadow-md rounded-lg p-4 mb-4">
                                        ...
                                        {demande.status === 'Acceptée' || demande.status === 'Accepted' ? (
                                            <>
                                                {isCurrent ? (
                                                    <>
                                                        <button className="bg-gray-500 text-white px-4 py-2 rounded mt-2 cursor-not-allowed">
                                                            En cours
                                                        </button>
                                                    ...
                                                    <>
                                                        <button className="bg-blue-500 text-white mr-2 px-4 py-2 rounded mt-2" disabled>
                                                            {demande.status}
                                                        </button>
