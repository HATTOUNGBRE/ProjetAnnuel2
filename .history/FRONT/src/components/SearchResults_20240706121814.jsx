...
const SearchResults = ({ searchTerm, maxPersons, handleShowDetails }) => {
  ...
  useEffect(() => {
    const fetchProperties = async () => {
      console.log('Fetching properties for search term:', searchTerm, 'and max persons:', maxPersons);
      try {
    ...
    if (searchTerm && maxPersons) {
      console.log('Search term and max persons provided, initiating fetch');
      fetchProperties();
    } else {
      console.log('No search term or max persons provided, skipping fetch');
      setProperties([]);
      setIsLoading(false);
    }
  }, [searchTerm, maxPersons]);
