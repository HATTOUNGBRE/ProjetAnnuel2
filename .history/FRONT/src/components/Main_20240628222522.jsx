...
function Main() {
  ...
  const handleSearch = async (e) => {
    e.preventDefault();
    const commune = searchTerm;
    const maxPersons = guests;

    try {
      const response = await fetch(
        `http://localhost:8000/api/search-properties?commune=${commune}&maxPersons=${maxPersons}`
      );
      const data = await response.json();
      setProperties(data);
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-pcs-100 flex flex-col items-center justify-center">
            ...
            <div className="relative w-1/4">
              ...
              {cities.length > 0 && (
                <ul className="absolute left-0 mt-2 w-full bg-white border rounded-lg z-10 max-h-48 overflow-y-auto">
                  {cities.map((city) => (
                    <li key={city.code} className="px-4 py-2 border-b last:border-b-0 cursor-pointer hover:bg-gray-200" onClick={() => handleCitySelect(city)}>
      ...
      <PropertySliderSection title="Toutes les Propriétés" apiUrl="http://localhost:8000/api/properties" sliderSettings={sliderSettings} />
      <PropertySliderSection title="Appartements Populaires" apiUrl="http://localhost:8000/api/popular-properties" sliderSettings={sliderSettings} />
      <PropertySliderSection title="Nouveaux Appartements" apiUrl="http://localhost:8000/api/new-properties" sliderSettings={sliderSettings} />
      <PropertySliderSection title="Appartements de Luxe" apiUrl="http://localhost:8000/api/luxury-properties" sliderSettings={sliderSettings} />
    </div>
  );
}
