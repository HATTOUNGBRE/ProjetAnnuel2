...
function Main() {
  ...
  return (
    <div className="min-h-screen bg-pcs-100 flex flex-col items-center justify-center">
            ...
            <div className="relative w-1/4">
              ...
              {cities.length > 0 && (
                <ul className="absolute left-0 mt-2 w-full bg-white border rounded-lg z-10 max-h-48 overflow-y-auto">
                  {cities.map((city) => (
                    <li key={city.code} className="px-4 py-2 border-b last:border-b-0 cursor-pointer hover:bg-gray-200" onClick={() => handleCitySelection(city)}>
      ...
      <PropertySliderSection title="Toutes les Propriétés" apiUrl="http://localhost:8000/api/properties" />
      <PropertySliderSection title="Appartements Populaires" apiUrl="http://localhost:8000/api/popular-properties" />
      <PropertySliderSection title="Nouveaux Appartements" apiUrl="http://localhost:8000/api/new-properties" />
      <PropertySliderSection title="Appartements de Luxe" apiUrl="http://localhost:8000/api/luxury-properties" />
    </div>
  );
}
