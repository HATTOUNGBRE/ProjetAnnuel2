  ...
  const handleSearch = async (e) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8000/api/search-properties?commune=${searchTerm}`
      );
      const data = await response.json();
      setSearchResults(data);
      if (data.length === 0) {
        setNoResultsMessage('Pas de résultat pour cette recherche');
      } else {
        setNoResultsMessage('');
      }
    } catch (error) {
    ...
    <div className="min-h-screen bg-pcs-100 flex flex-col items-center justify-center">
            ...
            <div className="relative w-1/2">
              ...
              {cities.length > 0 && (
                <ul className="absolute left-0 mt-2 w-full bg-white border rounded-lg z-10 max-h-48 overflow-y-auto">
                  {cities.map((city) => (
                    <li key={city.code} className="px-4 py-2 border-b last:border-b-0 cursor-pointer hover:bg-gray-200" onClick={() => handleCitySelect(city.nom)}>
      ...
      <div className="mt-10 w-full">
        ...
        {searchResults.length > 0 ? (
          <PropertyRow properties={searchResults} sliderSettings={sliderSettings} className="flex row" />
        ) : (
          <p className="text-xl text-gray-600">{noResultsMessage}</p>
        )}
      </div>
